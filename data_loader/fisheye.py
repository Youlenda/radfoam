...
def get_cam_ray_dirs_fisheye(camera):
    W = camera.width
    H = camera.height
    ...
    dist_sq = (pix_coords[:, 0] - cx)**2 + (pix_coords[:, 1] - cy)**2
    r_sq = r**2
    valid_mask = dist_sq <= r_sq
    
    valid_pix_coords = pix_coords[valid_mask]
    
    ip_coords = camera.cam_from_img(valid_pix_coords)
...
class Fisheye:
    def __init__(self, datadir, split, downsample):
        assert downsample in [1, 2, 4, 8] # TODO

        self.root_dir = datadir
        self.colmap_dir = os.path.join(datadir, "distorted/sparse/0/")
        self.split = split
        self.downsample = downsample

        if downsample == 1:
            images_dir = os.path.join(datadir, "input")
        else:
            images_dir = os.path.join(datadir, f"input_{downsample}")

        if not os.path.exists(images_dir):
            raise ValueError(f"Input directory {images_dir} not found")
        ...
        # dist_sq = ((xx + 0.5) - cx)**2 + ((yy + 0.5) - cy)**2 #TODO
